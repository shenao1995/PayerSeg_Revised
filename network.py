...
class Unet(tf.keras.Model):
    ...
    def __init__(self,
        ...
        self.data_format = data_format # [batch, channels, spatial_dim1, spatial_dim2, spatial_dim3]
        num_filters_base = num_filters_base
        if activation == 'relu':
            activation_fn = tf.nn.relu
        elif activation == 'lrelu':
            activation_fn = lambda x: tf.nn.leaky_relu(x, alpha=0.1) # LeakyReLU(x)=max(0,x) + alpha∗min(0,x)
