...
def run_graph(fetches, feed_dict=None):
    ...
    while len(node_queue) > 0:
        ...
        # 或者，如果你想保留检查，可以用鸭子类型检查（检查它有没有 get_parents 方法）
        if not hasattr(current_node, 'get_parents'):
            print(f"Warning: Node check failed for {current_node}, type: {type(current_node)}")
