...
def bb(image, transformation, image_spacing):
    ...
    start = np.flip(start.astype(np.float64) * np.array(image_spacing, np.float64))  # 切换坐标顺序
    end = np.flip(end.astype(np.float64) * np.array(image_spacing, np.float64))  # 切换坐标顺序
    start_transformed = transformation.TransformPoint(start)  # 指空间变换
    end_transformed = transformation.TransformPoint(end)  # 指空间变换
